      ...
      init: function() {
        ...
        if ( ( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) || (jQuery(window).width() < 768) ) {
            ...
            jQuery(window).scroll(function() {
                ...
                if (jQuery(".comment-reply-title").length > 0) {

                    var distanceTop = jQuery('.comment-reply-title').offset().top - jQuery(window).height();
        ...
        jQuery(".post-title a").each(function(){
	        if (jQuery(this).text().length > 46) {
	            jQuery(this).text(jQuery(this).text().substr(0, 43));
	            jQuery(this).append('...');
	        }
	    });
